<?php
/**
 * thegrove functions and definitions
 *
 ...
 */
function thegrove_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on thegrove, use a find and replace
		* to change 'thegrove' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'thegrove', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'thegrove' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'thegrove_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'thegrove_setup' );
 ...
 */
function thegrove_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'thegrove_content_width', 640 );
}
add_action( 'after_setup_theme', 'thegrove_content_width', 0 );
 ...
 */
function thegrove_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'thegrove' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'thegrove' ),
...
add_action( 'widgets_init', 'thegrove_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'thegrove_scripts' );
